...
export default function LogVitals() {
  ...
  const initialMetric =
    metric && validMetrics.includes(metric) ? metric : "weight";
  ...
  const history = useMemo(() => {
    ...
    const todayIdx = dashboardData.history.findIndex(
      (h) => h.date === todayStr,
    );
  ...
  const latestHistoryWeight = dashboardData?.history
    ?.slice()
    .reverse()
    .find((h) => h.weight > 0)?.weight;
  const currentWeight =
    selectedDateItem?.weight ||
    latestHistoryWeight ||
    user?.profile?.weight ||
    72.5;
  ...
  const sleepGoal = 8;
  const weightGoal =
    dashboardData?.goals?.weight || user?.nutritionGoal?.weightGoal || 70;
        ...
        const payload =
          activeTab === "steps"
            ...
            : {
                ...
                sleepData: {
                  totalSleepMinutes:
                    Number(sleepHours) * 60 + Number(sleepMins),
                  date,
                },
              };

        await api.post(
          activeTab === "steps" ? "wearables/sync" : "wearables/sleep",
          payload,
        );
      }

      toast.success(
        `${activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} logged successfully`,
      );
      ...
      <div className="p-6 md:p-8 bg-white/40 backdrop-blur-xl rounded-[32px] border border-white/60 shadow-[0_8px_32px_rgba(0,0,0,0.04)] mt-8 overflow-hidden relative">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 relative z-10">
          <div className="flex items-center gap-3.5">
            <div
              className={`w-12 h-12 rounded-2xl ${color} flex items-center justify-center shadow-inner`}
            >
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {insights && (
                  <span
                    className={`px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider border shadow-sm ${getStatusColor(insights.status)}`}
                  >
                    {insights.status}
                  </span>
                )}
        ...
        <div className="min-h-[120px] relative z-10">
          {insightsLoading && !insights ? (
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2.5">
                  {insights.recommendations?.map((rec, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-2 px-4 py-2.5 bg-white/60 hover:bg-white border border-slate-100/80 rounded-2xl shadow-sm transition-all hover:shadow-md cursor-default group"
                    >
            ...
            <div className="text-center py-10 bg-slate-50/50 rounded-3xl border border-dashed border-slate-200">
              ...
              <p className="text-sm text-slate-500 mb-6 font-semibold px-6">
                Ready to decode your {metricType} patterns?
              </p>
              <button
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-1.5 md:gap-2 px-2 md:px-6 py-2.5 md:py-3 rounded-full text-xs md:text-sm font-medium transition-all whitespace-nowrap relative ${
                    isActive
                      ? "text-[#1a1a1a] shadow-sm bg-white"
                      : "text-[#666666] hover:text-[#1a1a1a]"
                  }`}
                >
                  <Icon
                    className={`w-3.5 h-3.5 md:w-4 md:h-4 ${isActive ? "text-[#1a1a1a]" : "text-[#888888]"}`}
                  />
                ...
                >
                    ...
                    <div className="flex flex-col items-center justify-center p-6 md:p-8 bg-white/60 backdrop-blur-md rounded-[32px] border border-white shadow-sm relative overflow-hidden">
                      <div className="absolute top-6 right-6 flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-[#888888] tracking-widest uppercase">
                          {date === getLocalDateString() ? "LIVE" : "SELECTED"}
                        </span>
                      </div>
                      ...
                      <div className="relative w-48 h-48 flex items-center justify-center mb-8 bg-[#F5F5F7]/50 rounded-full border border-white border-dashed">
                        <div className="text-center z-10">
                          ...
                          <div className="text-[11px] font-bold text-[#888888] uppercase tracking-widest mt-2">
                            KILOGRAMS
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-3 w-full gap-4 pt-8 border-t border-slate-100/60 mt-4">
                        <div className="text-center">
                          <div className="text-xs font-bold text-[#888888] mb-1.5 uppercase tracking-wider">
                            Goal
                          </div>
                        ...
                        <div className="text-center border-l border-r border-slate-100/60">
                          ...
                          <div className="text-2xl font-medium text-[#1a1a1a]">
                            {(currentWeight - weightGoal).toFixed(1)}{" "}
                            <span className="text-sm text-[#888888] font-normal">
                              kg
                            </span>
                          </div>
                      ...
                      <div className="p-6 md:p-8 bg-[#F5F5F7]/50 rounded-[32px] border border-white border-dashed">
                        ...
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-4">
                          <div className="flex-1 w-full space-y-2">
                            <label className="text-[10px] font-bold text-[#888888] uppercase tracking-wider ml-2">
                              Weight (kg)
                            </label>
                            <input
                              type="number"
                          ...
                          <div className="flex-1 w-full space-y-2">
                            ...
                            <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl md:rounded-full px-4 py-3 shadow-sm">
                              ...
                              <input
                                ...
                                className="w-full bg-transparent text-[#1a1a1a] outline-none text-sm font-medium"
                              />
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="p-6 md:p-8 bg-white/60 backdrop-blur-md rounded-[32px] border border-white shadow-sm flex-1 flex flex-col mt-8">
                    <div className="flex justify-between items-center mb-8">
                      ...
                      <span className="px-4 py-1.5 bg-[#F5F5F7] text-[#666666] text-[10px] font-bold uppercase tracking-widest rounded-full border border-slate-100">
                        Last 7 Days
                      </span>
                    </div>
                    ...
                    <div className="flex flex-wrap justify-center gap-6 mt-6 pt-6 border-t border-slate-100/60">
                      <div className="flex items-center gap-2">
                        <div className="w-4 h-0 border-t-2 border-solid border-[#4A2B8C]"></div>
                        <span className="text-[10px] font-bold text-[#888888] uppercase tracking-wider">
                          Weight
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="w-4 h-0 border-t-2 border-dashed border-[#FF6B6B]"></div>
                        <span className="text-[10px] font-bold text-[#888888] uppercase tracking-wider">
                          Target ({weightGoal}kg)
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              {activeTab === "steps" && (
                ...
                >
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
                    <div className="flex flex-col items-center justify-center p-6 md:p-8 bg-white/60 backdrop-blur-md rounded-[32px] border border-white shadow-sm relative overflow-hidden">
                        ...
                        >
                          ...
                          <path
                            ...
                            strokeDashoffset={
                              120 *
                              Math.PI *
                              (1 - Math.min(currentSteps / stepGoal, 1))
                            }
                      ...
                      <div className="grid grid-cols-3 w-full gap-4 pt-8 border-t border-slate-100/60 mt-4">
                        ...
                        <div className="text-center border-l border-r border-slate-100/60">
                          ...
                          <div className="text-2xl font-medium text-[#1a1a1a]">
                            {(currentSteps * 0.00076).toFixed(1)}{" "}
                            <span className="text-sm text-[#888888] font-normal">
                              km
                            </span>
                          </div>
                    ...
                    <div className="flex flex-col gap-6">
                      <div
                        className={`p-6 md:p-8 rounded-[32px] border border-white border-dashed ${isToday ? "bg-[#F5F5F7]/50" : "bg-slate-50/50 grayscale-[0.5] opacity-80 cursor-not-allowed"}`}
                      >
                        <div className="flex items-center gap-3 mb-4">
                          <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center shadow-sm">
                            <Plus
                              className={`w-4 h-4 ${isToday ? "text-[#1a1a1a]" : "text-slate-400"}`}
                            />
                          </div>
                          <h4
                            className={`font-medium ${isToday ? "text-[#1a1a1a]" : "text-slate-500"}`}
                          >
                            {isToday ? "Add Steps" : "Log Steps"}
                          </h4>
                          {currentSteps > 0 && (
                            <span className="text-[10px] bg-white px-2 py-0.5 rounded-full border border-slate-200 text-slate-500 shrink-0 ml-1">
                              Total: {currentSteps.toLocaleString()}
                            </span>
                          )}
                          {!isToday && (
                            <span className="text-[9px] font-black uppercase text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-100 shrink-0">
                              Read Only
                            </span>
                          )}
                        </div>
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-4">
                          ...
                          <div className="flex-1 w-full space-y-2">
                            ...
                            <div
                              className={`flex items-center gap-2 bg-white border border-slate-200 rounded-xl md:rounded-full px-4 py-3 shadow-sm ${!isToday ? "bg-white/50" : ""}`}
                            >
                              <Calendar className="w-4 h-4 text-slate-400" />
                              <input
                                type="date"
                        ...
                        {!isToday && (
                          <p className="mt-4 text-[10px] font-bold text-slate-400 italic text-center">
                            Note: Historical steps cannot be edited manually.
                            Please select today to log.
                          </p>
                        )}
                  ...
                  <div className="p-6 md:p-8 bg-white/60 backdrop-blur-md rounded-[32px] border border-white shadow-sm flex-1 flex flex-col mt-8">
                          ...
                          >
                            ...
                            <YAxis
                              ...
                              domain={[
                                ...
                                (dataMax) =>
                                  Math.max(
                                    isFinite(dataMax) ? dataMax : stepGoal,
                                    stepGoal,
                                  ) * 1.15,
                              ]}
                            />
                    ...
                    <div className="flex flex-wrap justify-center gap-6 mt-6 pt-6 border-t border-slate-100/60">
                      <div className="flex items-center gap-2">
                        <div className="w-4 h-0 border-t-2 border-solid border-[#4A8C2B]"></div>
                        <span className="text-[10px] font-bold text-[#888888] uppercase tracking-wider">
                          Steps
                        </span>
                      </div>
              ...
              {activeTab === "sleep" && (
                ...
                >
                    ...
                    <div className="flex flex-col items-center justify-center p-6 md:p-8 bg-white/60 backdrop-blur-md rounded-[32px] border border-white shadow-sm relative overflow-hidden">
                      ...
                      <div className="text-center mb-8 mt-2">
                        <h2 className="text-2xl md:text-3xl font-light text-[#1a1a1a]">
                          ...
                          :{" "}
                          <span className="font-semibold text-[#1a1a1a]">
                            {currentSleepHrs}h {currentSleepMins}m
                          </span>
                        </h2>
                      </div>
                        ...
                        >
                          ...
                          <path
                            ...
                            strokeDashoffset={
                              120 *
                              Math.PI *
                              (1 - Math.min(currentSleep / sleepGoal, 1))
                            }
                            className="transition-all duration-1000 ease-out"
                          />
                        </svg>
                      ...
                      <div className="grid grid-cols-3 w-full gap-4 pt-8 border-t border-slate-100/60 mt-4">
                        <div className="text-center">
                          ...
                          <div className="text-2xl font-medium text-[#1a1a1a]">
                            {Math.floor(currentSleep * 0.3)}h{" "}
                            <span className="text-sm text-[#888888] font-normal">
                              {Math.round(((currentSleep * 0.3) % 1) * 60)}m
                            </span>
                          </div>
                        </div>
                        <div className="text-center border-l border-r border-slate-100/60">
                      ...
                      >
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <h4
                            className={`font-medium ${isToday ? "text-[#1a1a1a]" : "text-slate-500"}`}
                          >
                            {isToday ? "Add Sleep" : "Log Sleep"}
                          </h4>
                          {currentSleep > 0 && (
                            <span className="text-[10px] bg-white px-2 py-0.5 rounded-full border border-slate-200 text-slate-500 shrink-0 ml-1">
                              Current: {currentSleep.toFixed(1)} hrs
                            </span>
                          )}
                          {!isToday && (
                            <span className="text-[9px] font-black uppercase text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-100 shrink-0">
                        ...
                        {!isToday && (
                          <p className="mt-4 text-[10px] font-bold text-slate-400 italic text-center">
                            Note: Historical sleep data cannot be edited
                            manually. Please select today to log.
                          </p>
                        )}
                  ...
                  <div className="p-6 md:p-8 bg-white/60 backdrop-blur-md rounded-[32px] border border-white shadow-sm flex-1 flex flex-col mt-8">
                          ...
                          >
                            ...
                            <YAxis
                              ...
                              domain={[
                                ...
                                (dataMax) =>
                                  Math.max(
                                    isFinite(dataMax) ? dataMax : sleepGoal,
                                    sleepGoal,
                                  ) + 2,
                              ]}
                    ...
                    <div className="flex flex-wrap justify-center gap-6 mt-6 pt-6 border-t border-slate-100/60">
                      <div className="flex items-center gap-2">
                        <div className="w-4 h-0 border-t-2 border-solid border-[#75AADB]"></div>
                        <span className="text-[10px] font-bold text-[#888888] uppercase tracking-wider">
                          Sleep
                        </span>
                      </div>

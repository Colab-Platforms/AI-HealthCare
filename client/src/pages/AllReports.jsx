...
export default function AllReports() {
  ...
  const filteredReports = reports.filter((r) => {
    const matchesFilter =
      filter === "all" ||
      r.reportType.toLowerCase().includes(filter.toLowerCase());
    const matchesSearch =
      searchQuery === "" ||
      ...
      <div className="px-[21.96px] pt-4 flex flex-col gap-8 max-w-5xl mx-auto relative z-10">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="text-[14px] text-[#64748b] font-bold">
              Comprehensive archive of your diagnostic journey
            </p>
          </div>
        </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  {reports.length} Records Uploaded
                </span>
                <span className="text-[20px] font-black text-[#1a2138] tracking-tight uppercase">
                  Health Synthesis Active
                </span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            >
              <div
                className={`w-10 h-10 rounded-2xl ${stat.bg} ${stat.color} flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform`}
              >
                <stat.icon size={20} strokeWidth={2.5} />
              </div>
              <div className="flex flex-col">
                <span className="text-[20px] font-black text-[#1a2138] tracking-tight">
                  {stat.value}
                </span>
        ...
        <div className="flex gap-3 overflow-x-auto pb-4 no-scrollbar -mx-6 px-6 lg:mx-0 lg:px-0 scroll-smooth">
          {reportTypes.map((type) => (
            <button
              ...
              className={`px-6 py-3 rounded-full font-black whitespace-nowrap transition-all text-[11px] uppercase tracking-widest flex items-center gap-2.5 border-2 ${
                filter === type
            ...
            >
              <div
                className={`w-1.5 h-1.5 rounded-full ${filter === type ? "bg-[#69A38D]" : "bg-slate-200"}`}
              ></div>
          ...
          {filteredReports.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {filteredReports.map((report) => (
                <div
                  key={report._id}
                  className="bg-white/60 backdrop-blur-xl rounded-[40px] p-6 shadow-sm border border-white group relative hover:shadow-xl transition-all duration-500 overflow-hidden min-h-[220px] flex flex-col justify-between"
                >
                  ...
                  <div className="flex items-start justify-between relative z-10">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex flex-col">
                        ...
                        <h3 className="text-[18px] font-black text-[#1a2138] leading-tight uppercase tracking-tight line-clamp-1">
                          {report.reportType}
                        </h3>
                    ...
                    <div className="bg-white/80 rounded-[28px] p-4 flex items-center justify-between border border-[#69A38D]/10 mt-6 relative z-10">
                      <div className="flex flex-col">
                        ...
                        <div className="flex items-baseline gap-1.5">
                          <span className="text-3xl font-black text-[#1a2138] tracking-tighter">
                            {report.aiAnalysis.healthScore}
                          </span>
                      ...
                      <div className="flex -space-x-2">
                        {[1, 2, 3].map((i) => (
                          <div
                            key={i}
                            className="w-8 h-8 rounded-full border-2 border-white bg-slate-100 flex items-center justify-center text-[8px] font-black text-[#69A38D] shadow-sm uppercase"
                          >
                            {i}
                          </div>
                        ))}
            ...
            <div className="bg-white/60 backdrop-blur-xl rounded-[40px] p-20 border border-white flex flex-col items-center justify-center text-center gap-8 shadow-sm">
              ...
              <div className="flex flex-col gap-2 max-w-sm">
                ...
                <p className="text-[14px] text-slate-400 font-bold">
                  Start your digital diagnostic journey by uploading your first
                  clinical lab report.
                </p>
              </div>
              <Link
        ...
        <div className="text-center pt-8 pb-12 max-w-lg mx-auto">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-relaxed">
            Diagnostic archive is encrypted and HIPAA compliant. Processing
            engine utilizes clinical bio-marker synthesis for optimization
            insights.
          </p>
        </div>
      </div>

...
export default function AllReports() {
  ...
  return (
    <div className="w-full mx-auto space-y-8 animate-fade-in px-4 md:px-6 pt-2 md:pt-8 pb-24">
      {/* Welcome Message - Mobile Only */}
      <div className="md:hidden flex items-center justify-between">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <div className="w-10 h-10 rounded-full bg-black flex items-center justify-center text-white text-sm font-bold shadow-md flex-shrink-0">
      ...
      <div className="card card-gradient p-8 text-slate-800 shadow-2xl relative overflow-hidden ring-1 ring-white/50 border-none">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <Link to="/dashboard" className="inline-flex items-center gap-2 text-slate-500 hover:text-black font-black transition-all mb-4 text-[10px] uppercase tracking-widest group">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="card p-5 group hover:scale-[1.02] transition-all duration-300">
            <div className="flex flex-col items-center sm:items-start gap-4">
              <div className="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center shadow-inner group-hover:rotate-12 transition-transform">
        ...
        {reportTypes.map((type) => (
          <button
            ...
            className={`px-5 py-2.5 rounded-2xl font-black whitespace-nowrap transition-all text-[11px] uppercase tracking-widest leading-none snap-start border-2 ${filter === type
              ...
              : 'bg-white/50 backdrop-blur-sm text-slate-600 border-white hover:border-slate-400 hover:bg-white'
              }`}
          >
      ...
      {filteredReports.length > 0 ? (
            ...
            {filteredReports.map((report) => (
              ...
              >
                ...
                <div className="flex items-start gap-5 mb-6">
                  <div className="w-16 h-16 rounded-[1.25rem] bg-slate-100 flex items-center justify-center shadow-inner group-hover:bg-slate-200 transition-colors">
                  ...
                  <div className="flex-1">
                    ...
                    <h3 className="text-xl font-black text-slate-800 leading-tight uppercase group-hover:text-black transition-colors mb-1">
                      {report.reportType}
                    </h3>
                ...
                <Link
                  ...
                  className="w-full py-4 bg-white border-2 border-slate-100 text-slate-800 rounded-2xl font-black text-xs uppercase tracking-widest hover:border-black hover:text-black transition-all flex items-center justify-center gap-2 shadow-sm active:scale-95"
                >
                ...
                {filteredReports.map((report) => (
                  ...
                  >
                    ...
                    <div className="flex items-start gap-4 mb-4">
                      <div className="w-12 h-12 rounded-[1rem] bg-slate-100 flex items-center justify-center flex-shrink-0 shadow-inner">
                      ...
                      <div className="flex-1 min-w-0 pr-8">
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          <span className={`text-[8px] font-black px-2 py-0.5 rounded-md uppercase tracking-widest whitespace-nowrap ${report.status === 'completed' ? 'bg-slate-100 text-black' : 'bg-slate-50 text-slate-500'
                            }`}>
                            {report.status}
                          </span>
                        </div>
                    ...
                    <Link
                      to={`/reports/${report._id}`}
                      onClick={() => window.scrollTo(0, 0)}
                      className="w-full py-3 bg-white border-2 border-slate-100 text-slate-800 rounded-xl font-black text-[10px] uppercase tracking-widest hover:border-black hover:text-black transition-all flex items-center justify-center gap-2 shadow-sm active:scale-95"
                    >
                    ...
                    >
                      ...
                      <div className="flex items-start gap-4 mb-4 pr-8">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <span className={`text-[8px] px-2 py-0.5 rounded-md font-black uppercase tracking-widest inline-block ${report.status === 'completed' ? 'bg-slate-100 text-black' : 'bg-slate-50 text-slate-500'
                            }`}>
                            {report.status}
                          </span>
                        </div>
        ...
        <div className="text-center py-16 px-6 card border-none shadow-xl flex flex-col items-center max-w-lg mx-auto mt-12">
          <div className="w-24 h-24 bg-slate-100 rounded-3xl flex items-center justify-center mb-6 shadow-inner relative">
            <div className="absolute inset-0 bg-white/20 rounded-3xl blur" />
            <FileText className="w-12 h-12 text-slate-300 relative z-10" />
          </div>

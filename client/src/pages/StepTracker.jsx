...
export default function StepTracker() {
  ...
  const loadChartData = useCallback(() => {
    let data = [];
    try {
      data = JSON.parse(localStorage.getItem("takehealth_daily_steps") || "[]");
    } catch {}
    const chart = [];
    for (let i = 6; i >= 0; i--) {
      const d = new Date();
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-roboto w-full max-w-md mx-auto shadow-2xl overflow-hidden relative">
      ...
      <header className="bg-white px-6 py-3 md:py-5 flex items-center justify-between border-b border-slate-100 z-10 sticky top-0">
        ...
        <div className="flex items-center gap-1.5">
          <span
            className={`w-2 h-2 rounded-full ${sensorStatus === "active" ? "bg-black animate-pulse" : sensorStatus === "error" ? "bg-slate-300" : "bg-slate-400 animate-pulse"}`}
          />
      ...
      <div className="flex-1 overflow-y-auto pb-24">
            ...
            >
              ...
              <div className="text-left">
                <p className="text-sm font-black uppercase tracking-wider">
                  Tap to Enable Pedometer
                </p>
                <p className="text-[10px] opacity-80 font-bold">
                  Allow motion sensor access to count your steps
                </p>
              </div>
            </button>
          </div>
        ...
        {sensorStatus === "error" && (
          <div className="mx-6 mt-4 bg-red-50 text-red-500 text-xs font-bold p-4 rounded-2xl">
            ...
            <p>
              Step tracking requires a mobile device with accelerometer and
              gyroscope sensors.
            </p>
          </div>
        )}
        ...
        {sensorStatus === "active" && (
          <div className="mx-6 mt-3 bg-slate-50 rounded-xl px-4 py-2 flex items-center justify-between text-[10px] font-bold text-slate-400">
            ...
            <span
              className={debugInfo.gyroActive ? "text-black" : "text-slate-300"}
            >
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            >
              ...
              <p className="text-2xl font-black text-black group-hover:scale-105 transition-transform">
                {dailyGoal >= 1000
                  ? `${(dailyGoal / 1000).toFixed(dailyGoal % 1000 === 0 ? 0 : 1)}k`
                  : dailyGoal}
              </p>
            </div>
          </div>
        ...
        <div className="bg-white p-6 mt-3">
          ...
          <div className="flex items-center justify-center gap-6 mt-3 text-[9px] font-black uppercase tracking-widest text-slate-400">
            <span className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-sm bg-black" /> Goal Met
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-sm bg-slate-500" /> In
              Progress
            </span>
        ...
        {isEditingGoal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div
              ...
              className="bg-white w-full max-w-xs rounded-[2.5rem] p-8 relative z-10 shadow-2xl"
            >
              <h3 className="text-xl font-black text-black mb-1 tracking-tight">
                Set Daily Goal
              </h3>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 py-4 bg-black text-white text-xs font-black uppercase tracking-widest rounded-2xl shadow-lg shadow-slate-100"
                >
                  Save Goal
                </button>
              </div>

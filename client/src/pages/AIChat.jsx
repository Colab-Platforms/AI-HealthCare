...
export default function AIChat() {
  ...
  const saveChatToBackend = async (updatedMessages) => {
    try {
      console.log(`📤 Saving ${updatedMessages.length} messages to backend...`);
      
      // FIX: Send all messages, not just last 2 - deduplication happens on backend
      ...
      const response = await api.post("chat/history", { messages: messagesToSave });
      console.log(`✅ Backend save success:`, response.data);
      
      // Update localStorage immediately for offline access
      localStorage.setItem(
        `chat_history_${user?.id}`,
  ...
  const highlightText = (str) => {
    ...
    return words.map((word, i) => {
      ...
      if (["high", "spike", "danger", "reduce", "avoid"].includes(lower))
        return (
          <span key={i} className="text-rose-600 font-bold">
            {word}
          </span>
        );
      ...
      if (
        ["normal", "safe", "good", "healthy", "stable", "add"].includes(lower)
      )
        return (
      ...
      if (
        ...
        return (
          <span key={i} className="text-lg inline-block drop-shadow-sm">
            {word}
          </span>
        );
      }
    ...
    return (
      <div className="space-y-1.5 w-full">
        {lines.map((line, i) => {
          ...
          const emojisInLine = line.match(
            /[\u2700-\u27BF]|[\uE000-\uF8FF]|\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDFFF]|\uD83E[\uDD10-\uDDFF]/g,
          );
          const hasWarning =
            emojisInLine?.includes("⚠️") ||
            line.toLowerCase().includes("spike") ||
            line.toLowerCase().includes("high");
          const hasTime =
            emojisInLine?.includes("⏰") || line.toLowerCase().includes("time");
          const hasAction =
            emojisInLine?.includes("👉") ||
            line.toLowerCase().includes("reduce") ||
            line.toLowerCase().includes("add");
          const hasSuccess =
            emojisInLine?.includes("✅") ||
            line.toLowerCase().includes("safe") ||
            line.toLowerCase().includes("healthy");

          if (
            line.includes(":") &&
            line.split(":")[0].length < 35 &&
            (hasWarning || hasTime || hasAction || hasSuccess)
          ) {
            ...
            if (hasWarning) {
              Icon = TrendingUp;
              colorClass = "text-[#d62828] bg-[#fff5f5]";
              isSpikeGraph = true;
            } else if (hasTime) {
              Icon = History;
              colorClass = "text-[#e85d04] bg-[#fff8ee]";
            } else if (hasAction) {
              Icon = Zap;
              colorClass = "text-indigo-600 bg-[#f8f9ff]";
            } else if (hasSuccess) {
              Icon = Check;
              colorClass = "text-emerald-600 bg-[#f0fdf4]";
            }

            return (
              <div
                key={i}
                className={`flex flex-col my-2 p-4 rounded-2xl w-full transition-all ${colorClass}`}
              >
                ...
                {val && (
                  <span className="text-lg font-extrabold leading-relaxed text-slate-800">
                    {highlightText(val)}
                  </span>
                )}
                ...
                {isSpikeGraph && (
                  <div className="mt-4 pt-4 border-t border-rose-100/50 w-full">
                    <svg
                      viewBox="0 0 200 45"
                      className="w-full h-12 text-[#d62828] overflow-visible"
                    >
                      <defs>
                        <linearGradient
                          id="spikeGrad"
                      ...
                      />
                      <path
                        d="M 40 40 C 70 40, 80 5, 100 5 C 120 5, 130 40, 160 40"
                        fill="url(#spikeGrad)"
                      />
                      <path
                        d="M 40 40 C 70 40, 80 5, 100 5 C 120 5, 130 40, 160 40"
          ...
          if (line.trim().startsWith("•") || line.trim().startsWith("-")) {
            ...
            return (
              <div key={i} className="flex gap-3 items-start pl-1 py-1.5">
                ...
                <div className="flex-1 text-[15px] leading-relaxed font-bold text-slate-800">
          ...
          return (
            <p key={i} className="text-sm leading-relaxed font-medium py-1">
              {highlightText(line)}
            </p>
          );
        })}
  ...
  return (
    <div
      className="w-full bg-white flex flex-col md:flex-row md:pb-0"
      ...
      >
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-white">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-black text-black uppercase tracking-tighter">
              History
            </h2>
          </div>
          <button
      ...
      >
        ...
        >
          {messages.map((msg, i) => (
            ...
            >
              <div
                className={`flex gap-3 md:gap-4 w-full md:max-w-[75%] ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}
              >
                {msg.role === "assistant" && (
                  <div className="w-8 h-8 rounded-full shrink-0 flex items-center justify-center border border-slate-200 bg-white overflow-hidden">
                    <img
                      src="https://cdn.shopify.com/s/files/1/0636/5226/6115/files/icon.png?v=1775538354"
                      alt="Coach"
                ...
                <div
                  className={`relative w-full md:w-auto flex flex-col ${msg.role === "user" ? "bg-slate-100 text-slate-800 rounded-3xl px-5 py-3.5" : "bg-transparent text-slate-800 py-1 w-full max-w-none"}`}
                >
                  {msg.role === "user" ? (

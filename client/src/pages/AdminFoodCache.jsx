...
export default function AdminFoodCache() {
  ...
  const fetchFoods = async () => {
    setLoading(true);
    try {
      const { data } = await adminService.getFoodCache({
        search,
        page,
        limit: 15,
      });
  ...
  const handleClearAll = async () => {
    if (
      !window.confirm(
        "⚠️ WARNING: This will permanently delete EVERY food model in the Global Database. This action cannot be undone. Are you absolutely sure?",
      )
    )
      return;
                  ...
                  foods.map((food) => (
                    ...
                    >
                      <td className="px-6 py-4">
                        ...
                        <p
                          className={`text-[10px] font-black uppercase tracking-tighter ${food.isHealthy ? "text-emerald-500" : "text-orange-400"}`}
                        >
                          {food.isHealthy
                            ? "Healthy • IQ " + (food.healthScore || 0)
                            : "Caution • IQ " + (food.healthScore || 0)}
                        </p>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-xs font-medium text-slate-500 bg-slate-100 px-2 py-1 rounded-lg italic">
                          "{food.searchDescription}"
                        </span>
                      </td>
        ...
        {showModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative bg-white w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[95vh]"
            >
              <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between bg-white shrink-0">
                <h2 className="text-xl font-bold text-slate-800">
                  {isEditing
                    ? "Edit Food DB Entry"
                    : "Manual Food Add (AI Card Emulation)"}
                </h2>
                <button
                  onClick={() => setShowModal(false)}
              ...
              <form
                ...
                className="p-8 overflow-y-auto space-y-10 no-scrollbar"
              >
                {/* Basic Info */}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        className="w-full px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-sm outline-none focus:border-blue-500"
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        className="w-full px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-sm outline-none focus:border-blue-500"
                        placeholder="Exact text user searches"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-slate-500 uppercase">
                ...
                <div className="space-y-4 pt-4 border-t border-slate-50">
                  <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <Flame className="w-4 h-4" /> Nutrition Registry
                  </h3>
                      ...
                      <div key={macro.field} className="space-y-1.5">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              [macro.field]: parseFloat(e.target.value),
                            })
                          }
                ...
                <div className="space-y-4 pt-4 border-t border-slate-50">
                  <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <Zap className="w-4 h-4" /> AI Analysis Narrative
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-slate-500 uppercase">
                        Primary Analysis Card Body
                      </label>
                      <textarea
                        rows="4"
                        ...
                        className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-sm outline-none focus:border-blue-500 resize-none no-scrollbar"
                        placeholder="Deep health insight for the user..."
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-slate-500 uppercase">
                      ...
                      <textarea
                        rows="4"
                        value={formData.healthBenefitsSummary}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            healthBenefitsSummary: e.target.value,
                          })
                        }
                        className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-sm outline-none focus:border-blue-500 resize-none no-scrollbar"
                        placeholder="Brief summary of why this is good/bad..."
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10 pt-4 border-t border-slate-50">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        <Heart className="w-4 h-4" /> Key Benefits
                      </h3>
                      <button
                        type="button"
                        onClick={() =>
                          addItem("benefits", { name: "", benefit: "" })
                        }
                        className="p-1 px-2 text-[10px] font-bold bg-emerald-50 text-emerald-600 rounded-lg"
                      >
                        + Add
                      </button>
                    </div>
                    <div className="space-y-3">
                      {formData.benefits.map((item, idx) => (
                        <div
                          key={idx}
                          className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2 relative"
                        >
                          <button
                            type="button"
                          ...
                          <input
                            ...
                            placeholder="Benefit Title (e.g. Muscle Growth)"
                            value={item.name}
                            onChange={(e) => {
                          ...
                          />
                          <textarea
                            className="w-full bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-xs outline-none no-scrollbar"
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        <Activity className="w-4 h-4" /> Micronutrients
                      </h3>
                      <button
                        type="button"
                        onClick={() =>
                        ...
                        className="p-1 px-2 text-[10px] font-bold bg-blue-50 text-blue-600 rounded-lg"
                      >
                        + Add
                      </button>
                    </div>
                    <div className="space-y-2">
                      {formData.micronutrients.map((item, idx) => (
                        <div key={idx} className="flex gap-2 items-center">
                          ...
                          <input
                            type="number"
                            className="w-16 bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs"
                            placeholder="%"
                          ...
                          <button
                            type="button"
                            onClick={() => removeItem("micronutrients", idx)}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10 pt-4 border-t border-slate-50">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        <AlertTriangle className="w-4 h-4" /> Vital Warnings
                      </h3>
                      <button
                        type="button"
                        onClick={() => addItem("warnings", "")}
                        className="p-1 px-2 text-[10px] font-bold bg-orange-50 text-orange-600 rounded-lg"
                      >
                        + Add
                      </button>
                    </div>
                      ...
                      {formData.warnings.map((item, idx) => (
                        <div key={idx} className="flex gap-2">
                          <input
                            ...
                            value={item}
                            onChange={(e) =>
                              updateArrayItem("warnings", idx, e.target.value)
                            }
                          />
                          <button
                            type="button"
                            onClick={() => removeItem("warnings", idx)}
                            className="text-rose-400"
                          >
                            <X className="w-4 h-4" />
                          </button>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        <HelpCircle className="w-4 h-4" /> Healthy Tips
                      </h3>
                      <button
                        type="button"
                        onClick={() =>
                          addItem("enhancementTips", { name: "", benefit: "" })
                        }
                        className="p-1 px-2 text-[10px] font-bold bg-slate-100 text-slate-600 rounded-lg"
                      >
                        + Add
                      </button>
                    </div>
                        ...
                        <div key={idx} className="flex gap-2">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              setFormData({
                                ...formData,
                                enhancementTips: newT,
                              });
                            }}
                          />
                          <button
                            type="button"
                            onClick={() => removeItem("enhancementTips", idx)}
                            className="text-rose-400"
                          >
                            <X className="w-4 h-4" />
                          </button>
                  ...
                  <div className="col-span-1 md:col-span-2 space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        <List className="w-4 h-4" /> Healthy Alternatives
                      </h3>
                      <button
                        type="button"
                        onClick={() =>
                          addItem("alternatives", { name: "", description: "" })
                        }
                        className="p-1 px-2 text-[10px] font-bold bg-purple-50 text-purple-600 rounded-lg"
                      >
                        + Add
                      </button>
                    </div>
                    <div className="space-y-3">
                      {formData.alternatives.map((item, idx) => (
                        ...
                        >
                          <button
                            type="button"
                            onClick={() => removeItem("alternatives", idx)}
                            className="absolute top-2 right-2 text-rose-400"
                          >
                            <X className="w-4 h-4" />
                          </button>
                          <input
                            className="w-full bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-xs font-bold"
                            placeholder="Alternative Name (e.g. Quinoa)"
                            value={item.name}
                            onChange={(e) => {
                          ...
                          <textarea
                            className="w-full bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-xs outline-none no-scrollbar"
                            placeholder="Why is this a better choice?..."
                ...
                <div className="pt-6 border-t border-slate-50 flex items-center gap-4">
                  <button
                    ...
                    className={`flex-1 py-4 px-6 rounded-2xl text-xs font-bold border transition-all flex items-center justify-center gap-3 ${formData.isHealthy ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-orange-50 text-orange-600 border-orange-100"}`}
                  >
                    {formData.isHealthy ? (
                ...
                <div className="flex gap-4 pt-4 font-bold text-sm shrink-0">
                  ...
                  <button
                    type="submit"
                    className="flex-[2] py-4 bg-blue-600 text-white rounded-2xl hover:bg-blue-700 transition-all shadow-xl shadow-blue-100 flex items-center justify-center gap-2"
                  >
        ...
        {showBulkModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="relative bg-white w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh]"
            >
              <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between bg-white shrink-0">
                <h2 className="text-xl font-bold text-slate-800">
                  Bulk Import Food Models
                </h2>
                <button
                  onClick={() => setShowBulkModal(false)}
                  className="text-slate-400 hover:text-slate-600 transition-colors"
              ...
              <form
                ...
                className="p-8 space-y-6 overflow-y-auto no-scrollbar"
              >
                <div className="bg-blue-50 border border-blue-100 p-4 rounded-2xl flex items-start gap-4">
                  ...
                  <div className="text-xs text-blue-700 leading-relaxed">
                    ...
                    <p>
                      Upload a JSON array of food objects. Each object should
                      follow the standard Food Model schema used in manual
                      addition.
                    </p>
                    <button
                      type="button"
                      onClick={downloadTemplate}
                      className="mt-2 flex items-center gap-1.5 font-bold underline hover:text-blue-900 transition-colors"
                    >
                ...
                <div className="flex gap-4 pt-2">
                  ...
                  <button
                    type="submit"
                    className="flex-[2] py-4 bg-blue-600 text-white rounded-2xl font-bold text-sm hover:bg-blue-700 transition-all shadow-xl shadow-blue-100 flex items-center justify-center gap-2"
                  >

...
export default function DiabetesCare() {
  ...
  const fetchAiAnalysis = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        ...
        setAiInsight(
          "Your metabolic data is being processed. Complete more logs for a deep-dive analysis.",
        );
      }
    } catch (error) {
      setAiInsight(
        "Logging fresh data will allow AI to generate a detailed analysis of your metabolic spikes.",
      );
    } finally {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const allReadings = [
        ...
        (a, b) =>
          new Date(b.recordedAt || b.createdAt) -
          new Date(a.recordedAt || a.createdAt),
      );

      setRecentLogs(allReadings);

      const fastingReadings = glucoseReadings.filter(
        (r) => (r.readingContext || "").toLowerCase() === "fasting",
      );
      if (fastingReadings.length > 0) {
        const avg = Math.round(
          fastingReadings.reduce((sum, r) => sum + (r.value || 0), 0) /
      ...
      glucoseReadings
        ...
        .forEach((r) => {
          ...
          else if (ctx === "premeal" || ctx === "pre meal")
            dayMap[dayKey].preMeal = r.value;
          else if (ctx === "postmeal" || ctx === "post meal")
            dayMap[dayKey].postMeal = r.value;
          else if (ctx === "random") dayMap[dayKey].random = r.value;
        });
      setGlucoseChartData(Object.values(dayMap).slice(-7));

      const hba1cChart = hba1cReadings
        .slice(0, 6)
        .reverse()
        .map((r) => ({
          month: new Date(r.recordedAt || r.createdAt).toLocaleDateString(
            "en-US",
            { month: "short" },
          ),
          value: r.value,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.post("metrics", {
        ...
        readingContext:
          category === "HbA1c" ? "lab" : glucoseContext.toLowerCase(),
        recordedAt,
      });
  ...
  const getLogStatus = (reading) => {
    ...
    if (ctx === "fasting")
      return reading.value <= 99
        ? "good"
        : reading.value <= 125
          ? "warning"
          : "high";
    if (ctx.includes("post"))
      return reading.value <= 140
        ? "good"
        : reading.value <= 199
          ? "warning"
          : "high";
  ...
  return (
    <div
      className={`min-h-screen ${theme.background} ${theme.darkBackground} transition-colors pb-32`}
    >
      <SEO pageName="diabetes" />
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4 mb-2">
              <div className="flex flex-col gap-1">
                <h2 className="text-[32px] font-semibold text-slate-900 dark:text-white tracking-tight leading-none">
                  Diabetes Log
                </h2>
                <p className="text-[13px] font-semibold text-slate-500 leading-relaxed">
                  Comprehensive tracking for Glucose and HbA1c metrics.
                </p>
              </div>
              ...
              <div className="flex gap-4">
                <div className="flex-1 bg-white/70 dark:bg-white/5 backdrop-blur-xl border border-white/60 dark:border-white/10 rounded-[20px] p-4 flex flex-col justify-center shadow-none">
                  ...
                  <div className="flex items-baseline gap-1">
                    <span className="text-[24px] font-semibold text-[#69A38D] leading-none">
                      {avgFasting}
                    </span>
            ...
            <div className="bg-white/70 dark:bg-white/5 backdrop-blur-xl border border-white/60 dark:border-white/10 rounded-[24px] p-4 shadow-none flex flex-col gap-4">
              <h3 className="text-[16px] font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                <Plus size={18} className="text-[#69A38D]" strokeWidth={2.5} />{" "}
                Log New Reading
              </h3>
                ...
                <div>
                  ...
                  {category === "Glucose" && (
                    <div className="grid grid-cols-4 lg:grid-cols-2 gap-2 mt-3">
                      {["Fasting", "Pre-Meal", "Post-Meal", "Random"].map(
                        (ctx) => (
                          <button
                            key={ctx}
                ...
                <div className="flex gap-4">
                  <div className="flex-1">
                    ...
                    >
                      <Calendar
                        size={16}
                        className="text-slate-400 group-hover:text-[#69A38D] transition-colors shrink-0"
                      />
                      <input
                        ref={dateInputRef}
                        type="date"
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className={theme.card + " p-6 md:p-8"}>
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                <div>
                  <h3 className="text-[18px] font-black text-[#1a2138] dark:text-white uppercase tracking-tight mb-1">
                    Health Trends
                  </h3>
                ...
                <div className="flex bg-slate-100/50 dark:bg-black/20 p-1 rounded-2xl border border-white/50">
                  <button
                    ...
                    className={`px-5 py-2 text-[11px] font-black uppercase rounded-xl transition-all ${activeChart === "glucose" ? "bg-white dark:bg-[#2D2D2D] text-[#6FAF95] shadow-xs" : "text-slate-400 hover:text-slate-600"}`}
                  >
                    Daily
                  </button>
                  <button
                    onClick={() => setActiveChart("hba1c")}
                    className={`px-5 py-2 text-[11px] font-black uppercase rounded-xl transition-all ${activeChart === "hba1c" ? "bg-white dark:bg-[#2D2D2D] text-[#6FAF95] shadow-xs" : "text-slate-400 hover:text-slate-600"}`}
                  >
                    HbA1c
                  </button>
                </div>
              ...
              <div className="flex flex-col gap-3 max-h-[280px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-[#69A38D]/20">
                {loading ? (
                  <div className="py-12 flex flex-col items-center gap-3">
                    <Loader2
                      size={24}
                      className="animate-spin text-[#69A38D]"
                    />
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div
                            className={`w-14 h-14 rounded-2xl border-2 flex flex-col items-center justify-center ${statusColors[status]}`}
                          >
                            <span className="text-[18px] font-black leading-none">
                              {log.value}
                            </span>
                            <span className="text-[9px] font-black uppercase mt-0.5">
                              {log.type === "HbA1c" ? "%" : "mg/dL"}
                            </span>
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="text-[14px] font-black text-[#1a2138] dark:text-white uppercase tracking-tight">
                                {log.type}
                              </span>
                              <span
                                className={`text-[8px] font-black px-2 py-0.5 rounded-full uppercase border ${statusColors[status]}`}
                              >
            ...
            <div className="mt-2 bg-[#69A38D] dark:bg-[#5B8D7A]/90 rounded-[32px] p-6 md:p-8 border border-white/10 relative transition-all group shadow-lg">
              <div className="relative z-10">
                <div className="flex justify-end mb-4">
                  ...
                  >
                    <RefreshCw
                      size={14}
                      className={analyzingAi ? "animate-spin" : ""}
                    />
                  </button>
                </div>
                ...
                <div className="flex flex-col gap-5">
                    ...
                    <p className="text-[15px] font-medium text-white/95 leading-relaxed mb-4 border-l-2 border-white/40 pl-4">
                      {analyzingAi ? (
                        <span className="flex items-center gap-2">
                          ...
                          />{" "}
                          Analyzing your metabolic patterns...
                        </span>
                      ) : (
                      ...
                      <div className="flex flex-col gap-3.5 mt-4">
                        {aiFullData.recommendations.map((rec, idx) => (
                          ...
                          >
                            ...
                            <span className="text-[14px] font-medium text-white/90 leading-tight w-full">
                              {rec}
                            </span>
                          </div>
                        ))}
                  ...
                  {aiFullData?.immediateAction && (
                    <div className="mt-2 pt-5 border-t border-white/10 flex flex-col gap-1 w-full">
                      <span className="text-[10px] font-black text-white/50 uppercase tracking-[0.2em] block">
                        Immediate Step
                      </span>

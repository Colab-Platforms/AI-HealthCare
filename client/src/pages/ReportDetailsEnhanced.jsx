...
import { FileText, Activity, Apple, Heart, AlertTriangle, ArrowLeft, Sparkles, Droplets, ChevronDown, ChevronUp, Dumbbell, TrendingUp, TrendingDown, BarChart2, Languages, CheckCircle2, Zap } from 'lucide-react';
import toast from 'react-hot-toast';
import {
  LineChart, Line, AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip as RechartTooltip,
...
export default function ReportDetailsEnhanced() {
  ...
  useEffect(() => {
    if (activeTab !== 'trends' || trendsData) return;
    const loadTrends = async () => {
      setTrendsLoading(true);
      try {
        const params = report?.reportType ? { reportType: report.reportType } : {};
        const { data } = await healthService.getTrends(params);
        setTrendsData(data);
        if (data.availableMetrics?.length) setSelectedTrendMetric(data.availableMetrics[0]);
      } catch {
      ...
      {activeTab === 'trends' ? (
        ...
        <div className="space-y-8">
          ...
          <div className="card p-8 border-none ring-1 ring-purple-100">
            ...
            ) : trendsData?.healthScoreTrend?.length > 1 ? (
              <ResponsiveContainer width="100%" height={260}>
                <AreaChart data={trendsData.healthScoreTrend.map(d => ({
                  ...d,
                  ...
                  />
                  <ReferenceLine y={70} stroke="#10b981" strokeDasharray="4 4" label={{ value: 'Good', fill: '#10b981', fontSize: 10, fontWeight: 700 }} />
                  <Area type="monotone" dataKey="score" stroke="#9333ea" strokeWidth={2.5} fill="url(#scoreGrad)" dot={{ fill: '#9333ea', strokeWidth: 0, r: 4 }} activeDot={{ r: 6 }} />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              <div className="flex flex-col items-center justify-center h-48 text-slate-400">
                <BarChart2 className="w-10 h-10 mb-3 opacity-30" />
                <p className="text-sm font-bold">Need at least 2 reports to show trend</p>
              </div>
            )}
          </div>

          {/* Per-Metric Trend */}
          <div className="card p-8 border-none ring-1 ring-cyan-100">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              ...
              {trendsData?.availableMetrics?.length > 0 && (
                <select
                  value={selectedTrendMetric}
                  onChange={e => setSelectedTrendMetric(e.target.value)}
                  className="text-[11px] font-black uppercase tracking-wider border border-slate-200 rounded-xl px-4 py-2.5 bg-white text-slate-700 focus:outline-none focus:ring-2 focus:ring-cyan-300"
                >
            ...
            {trendsLoading ? (
              ...
              const series = trendsData?.metricTrends?.[selectedTrendMetric] || [];
              if (series.length < 2) return (
                <div className="flex flex-col items-center justify-center h-48 text-slate-400">
                  <Activity className="w-10 h-10 mb-3 opacity-30" />
                  <p className="text-sm font-bold">Not enough data points for this metric</p>
                  <p className="text-xs mt-1">Upload more {report?.reportType} reports to see trends</p>
                </div>
              );
              ...
              const statusColor = { normal: '#10b981', borderline: '#f59e0b', abnormal: '#ef4444' };
              const unit = series[0]?.unit || '';
              const normalRange = series[0]?.normalRange || '';
                ...
                <div className="space-y-4">
                  {normalRange && (
                    <div className="flex items-center gap-2 text-[11px] font-black text-slate-500">
                      ...
                      Normal Range: <span className="text-emerald-600">{normalRange} {unit}</span>
                    </div>
                  )}
                  <ResponsiveContainer width="100%" height={260}>
                    <LineChart data={chartData}>
                      ...
                      <RechartTooltip
                        contentStyle={{ borderRadius: 12, border: '1px solid #e2e8f0', fontSize: 12, fontWeight: 700 }}
                        formatter={(v) => [`${v} ${unit}`, selectedTrendMetric.replace(/([A-Z])/g, ' $1').trim()]}
                      />
                      <Line
                        type="monotone"
                        dataKey="value"
                        ...
                        strokeWidth={2.5}
                        dot={(props) => {
                          const s = series[props.index]?.status || 'normal';
                          return <circle key={props.index} cx={props.cx} cy={props.cy} r={5} fill={statusColor[s] || '#06b6d4'} stroke="#fff" strokeWidth={2} />;
                        }}
                        activeDot={{ r: 7 }}
                      />
                    </LineChart>
                  ...
                  <div className="flex flex-wrap gap-3 pt-2">
                    {[['normal', 'bg-emerald-500', 'Normal'], ['borderline', 'bg-amber-500', 'Borderline'], ['abnormal', 'bg-red-500', 'Abnormal']].map(([s, bg, label]) => (
                  ...
                  <div className="grid grid-cols-3 gap-4 mt-4">
                    {[
                      { label: 'Latest', value: `${series[series.length - 1]?.value} ${unit}`, color: statusColor[series[series.length - 1]?.status] },
                      { label: 'First Recorded', value: `${series[0]?.value} ${unit}`, color: '#64748b' },
                      { label: 'Data Points', value: series.length, color: '#9333ea' }

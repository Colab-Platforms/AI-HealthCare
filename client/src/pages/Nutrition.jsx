...
function Nutrition() {
  ...
  const [loading, setLoading] = useState(
    !cachedNutritionData && !cachedNutritionLogs,
  );
  ...
  const [selectedRecipeSuggestion, setSelectedRecipeSuggestion] =
    useState(null);
  ...
  const currentSuggestion = React.useMemo(() => {
    ...
    const isVeg =
      ...
      user?.profile?.dietaryPreference === "vegan" ||
      user?.profile?.dietaryPreference === "eggetarian";
    ...
    const suggestionsMap = {
      Recommended: isVeg
        ? {
            ...
            reason:
              "A wholesome balanced meal with complete plant proteins for your daily energy.",
            image: null,
            ingredients: [
              "Yellow Moong Dal",
              "Basmati Rice",
            ...
            instructions: [
              ...
              'Perform "Tadka" with ghee, cumin and chilies.',
              "Steam rice until fluffy and serve together.",
            ],
          }
        : {
            ...
            reason:
              "Traditional Indian style fish curry to support your heart health and protein goals.",
            image: null,
            ingredients: [
              "Fresh Fish Fillet",
              "Onion-Tomato Masala",
            ...
            instructions: [
              "Sauté onion and tomato paste with spices.",
              "Add fish pieces and simmer in curry base.",
              "Serve with warm cooked brown rice.",
            ],
          },
      "High Protein": isVeg
        ? {
            ...
            reason:
              "Specifically suggested to help reach your protein target of 70g using fresh paneer.",
            image: null,
            ingredients: [
              "Crumbled Paneer (150g)",
              "Whole Wheat Roti",
              "Chopped Onions",
              "Bell Peppers",
              "Garam Masala",
            ...
            instructions: [
              "Heat oil and sauté onions and peppers.",
              "Add crumbled paneer and spice mix.",
              "Serve with freshly made soft wheat rotis.",
            ],
          }
        : {
            ...
            reason:
              "High protein, low carb meal to support muscle recovery without extra calories.",
            image: null,
            ingredients: [
            ...
            instructions: [
              "Marinate chicken in curd and spices for 30 mins.",
              "Air fry or grill until charred and juicy.",
              "Serve with a fresh squeeze of lemon and salad.",
            ],
          },
      Balanced: {
        ...
        reason:
          "A light, gut-healing meal that balances all necessary macronutrients.",
        image: null,
        ingredients: [
          "Rice & Lentils Mix",
          "Carrots",
        ...
        instructions: [
          "Cook rice, lentils and chopped veg in a pressure cooker.",
          "Finish with a tiny dollop of ghee.",
          "Serve with a bowl of refreshing cold curd.",
        ],
      },
      "Low Carb": isVeg
        ? {
            ...
            reason:
              "Simple Indian egg scramble to keep you full without traditional heavy carbs.",
            image: null,
            ingredients: [
              "3 Whole Eggs",
              "Onions",
            ...
            instructions: [
              ...
              "Sauté onions and chilies until soft.",
              "Add eggs and scramble until moist and fully cooked.",
            ],
          }
        : {
            ...
            reason:
              "A low-glycemic choice that helps maintain stable blood sugar levels.",
            image: null,
            ingredients: [
              "2 Eggs",
              "Green Sprouted Moong",
              "Spinach",
              "Onions",
              "Black Pepper",
            ],
            instructions: [
              "Prepare a spicy masala omelette with veggies.",
              "Steamed sprouts served on the side as a fiber source.",
            ],
          },
    };
      ...
      if (topDef.includes("iron") || topDef.includes("hemoglobin")) {
        suggestionsMap["Recommended"] = {
          ...
          instructions: [
            ...
            "Combine and temper with garlic ghee.",
          ],
        };
        ...
        suggestionsMap["Recommended"] = {
          ...
          instructions: [
            "Soak and pressure cook bajra and dal mix.",
            "Whisk buttermilk with roasted cumin powder.",
        ...
        suggestionsMap["Recommended"] = {
          ...
          reason:
            "Protein synthesis: Suggested to support your muscle recovery after your recent activity.",
        };
      }
    }
  ...
  useEffect(() => {
    ...
    if (location.state?.prefillData) {
      ...
      setMealTab(
        p.mealType
          ? p.mealType.charAt(0).toUpperCase() + p.mealType.slice(1)
          : "Breakfast",
      );
      setAnalysisResult({
        _id: "new_log_again",
  ...
  const fetchData = async (force = false) => {
    ...
    try {
      ...
      if (logs) {
        logs.forEach((log) => {
          ...
          if (type.toLowerCase().includes("breakfast"))
            grouped.Breakfast.push(log);
          else if (type.toLowerCase().includes("lunch"))
            grouped.Lunch.push(log);
      ...
      if (bCals > bT)
        ...
        insight =
          "Lunch was calorie-dense. A high-protein dinner would be ideal.";
      else if (dCals > dT)
        insight = "Dinner exceeded target. Consider an active start tomorrow.";
      else if (summary?.totalCalories > 0)
        insight = "Excellent! You're managing your meal portions very well.";
  ...
  const handleWaterUpdate = async (change) => {
    try {
      ...
      if (newWater > waterIntake.target) {
        ...
        toast(
          "You've completed your daily target of 8 glasses. Stay hydrated, but you're all set!",
          {
            icon: "✨",
            duration: 4000,
          },
  ...
  const handleAnalyzeAndLog = async () => {
    ...
    try {
      ...
      if (!name || name === "Unknown Food" || (totalCals === 0 && !isWater)) {
        toast.error(
          "Food not detected. Please upload a clearer image of your meal.",
          {
            icon: "🍽️",
            duration: 4000,
          },
  ...
  const handleMoveMeal = async (id, newType) => {
    try {
      ...
      const newKey =
        newType === "breakfast"
          ? "Breakfast"
          : newType === "midMorningSnack"
            ? "Mid-Morning"
            : newType === "lunch"
              ? "Lunch"
              : newType === "eveningSnack"
                ? "Evening"
                : "Dinner";
      ...
      toast
        .promise(api.put(`nutrition/logs/${id}`, { mealType: newType }), {
          loading: "Moving meal...",
  ...
  const remainingCals = Math.max(
    0,
    dailySummary.calorieTarget - dailySummary.caloriesConsumed,
  );
  const progressPercent = Math.min(
    100,
    (dailySummary.caloriesConsumed / dailySummary.calorieTarget) * 100,
  );
      ...
      <div className="container mx-auto px-4 pt-2 pb-8">
        <NutritionTab
          onLogFood={async (mode, mealType, file) => {
            const inputMode =
              mode === "photo" ||
              mode === "Add Food via Photo" ||
              mode === "import" ||
              mode === "Import Image"
                ? "Scan"
                : mode === "voice" || mode === "Voice Log"
                  ? "Predict"
                  : mode === "text" || mode === "Type" || mode === "Text Only"
                    ? "Type"
                    : mode === "Scan"
                      ? "Scan"
                      : "Scan";
              ...
              <div className="p-6 md:p-8 pb-4">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button
                    ...
                    className="w-10 h-10 bg-white hover:bg-slate-50 rounded-full flex items-center justify-center transition-all border border-slate-200 shadow-sm shrink-0"
                  >
                    ...
                    >
                      {tab.icon && (
                        <tab.icon
                          size={14}
                          className={
                            mealTab === tab.id ? "text-white" : "text-slate-400"
                          }
                        />
                      )}
              ...
              <div className="px-6 md:px-8 pb-10 md:pb-8 overflow-y-auto flex-1 scrollbar-hide flex flex-col">
                {isAnalyzing ? (
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="flex flex-col items-center justify-center space-y-6 h-full py-12 flex-1"
                  >
                    ...
                    <div className="bg-[#5c7a6e] rounded-[2rem] p-6 lg:p-8 flex flex-col items-center justify-center relative shadow-inner w-full border-2 border-dashed border-white/20 flex-1 min-h-[250px] overflow-hidden">
                      {imagePreview ? (
                        ...
                        <>
                          ...
                          <button
                            onClick={() => {
                              if (cameraModalInputRef.current)
                                cameraModalInputRef.current.value = "";
                          ...
                          <span className="text-white/60 text-[10px] font-black uppercase mb-4 tracking-[0.2em]">
                            OR
                          </span>
                          ...
                          <button
                            onClick={() =>
                              document
                                .getElementById("food-img-upload")
                                ?.click()
                            }
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8">
                      <div className="space-y-2.5">
                        <label className="text-[10px] font-black text-[#5c806d] uppercase tracking-widest pl-1">
                          Quantity
                        </label>
                        <input
                          type="text"
                    ...
                    <div className="mb-8 w-full text-left">
                      <p className="text-[10px] font-black text-[#5c806d] uppercase tracking-widest pl-1 mb-3.5">
                        Quick Search Tags
                      </p>
                  ...
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="flex flex-col items-center justify-center space-y-6 flex-1 py-4"
                  >
                    <div className="relative mb-4">
                      {isListening && (
                        <>
                          ...
                          <div
                            className="absolute -inset-4 rounded-full bg-[#69A38D] opacity-10 animate-ping"
                      ...
                      >
                        <Mic
                          className={`w-12 h-12 ${isListening ? "animate-pulse" : ""}`}
                        />
                      </button>
                    </div>
                    ...
                    {foodInput && (
                      <div className="w-full space-y-5 flex-1 flex flex-col">
                        <div className="bg-slate-50 p-6 rounded-3xl border border-slate-200 min-h-[120px] flex items-center justify-center relative shadow-inner">
                          <p className="text-base font-bold text-slate-800 leading-relaxed text-center w-full">
                            {foodInput}
                          </p>
                          {isListening && (
                            <div className="w-3 h-3 bg-red-500 rounded-full absolute bottom-4 right-4 animate-pulse shadow-sm" />
                          )}
                        </div>
                ...
                <div className="flex justify-between p-5 bg-slate-50 rounded-[2rem] border border-slate-100 mb-8">
                  <div className="text-center flex-1 border-r border-slate-200">
                    ...
                    <p className="text-lg font-black text-slate-900">
                      {selectedRecipeSuggestion.calories}
                    </p>
                  </div>
                  <div className="text-center flex-1 border-r border-slate-200">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">
                    ...
                    <p className="text-lg font-black text-slate-900">
                      {selectedRecipeSuggestion.protein}g
                    </p>
                  </div>
                    ...
                    <div className="space-y-5">
                      {selectedRecipeSuggestion.instructions.map((step, i) => (
                        <div key={i} className="flex gap-4 group">
                          <span className="text-sm font-black text-slate-200 group-hover:text-slate-900 transition-colors">
                            0{i + 1}
                          </span>

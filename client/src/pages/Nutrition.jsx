...
export default function Nutrition() {
  ...
  return (
    <div className="space-y-6 animate-fade-in w-full overflow-x-hidden">
      ...
      {/* Quick Food Check - Responsive */}
      <div className="bg-white rounded-2xl p-3 md:p-6 shadow-sm border border-[#E5DFD3] w-full overflow-hidden">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <Search className="w-5 h-5 text-[#5C4F3D]" />
            <h2 className="text-lg md:text-xl font-semibold text-[#2C2416]">Quick Food Check</h2>
          </div>
          {quickCheckResult && (
            <button
              onClick={clearQuickCheck}
          ...
          <div className="space-y-3">
            <div className="relative">
              <input
                type="text"
                value={quickCheckFood}
                onChange={handleFoodInputChange}
                onKeyPress={(e) => e.key === 'Enter' && handleQuickCheck()}
                placeholder="E.g., Pizza, 2 slices or Milk, 250ml"
              ...
              {showQuantitySuggestion && suggestedUnit && (
                <div className="absolute top-full left-0 right-0 mt-1 p-3 bg-blue-50 border-2 border-blue-200 rounded-xl shadow-lg z-10">
                  <div className="flex items-start gap-2">
                    <Lightbulb className="w-4 h-4 text-blue-600 mt-0.5 flex-shrink-0" />
                    <p className="text-xs md:text-sm text-blue-700">{suggestedUnit}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="flex gap-2">
              <label className="flex-1 flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed border-[#E5DFD3] rounded-xl hover:border-[#8B7355] cursor-pointer transition-colors">
                <Camera className="w-5 h-5 text-[#5C4F3D]" />
                <span className="text-sm text-[#5C4F3D] hidden md:inline">
                  {quickCheckImage ? 'Image Selected ✓' : 'Photo'}
                </span>
                <span className="text-xs text-[#5C4F3D] md:hidden">
                  {quickCheckImage ? 'Image ✓' : 'Photo'}
                </span>
                <input
                  type="file"
                  accept="image/*"
                  onChange={handleQuickCheckImageSelect}
                  className="hidden"
                />
              </label>
              ...
              <button
                ...
                disabled={checkingFood}
                className="px-6 py-3 bg-[#8B7355] text-white rounded-xl font-medium hover:bg-[#A0826D] disabled:opacity-50 flex items-center gap-2 text-sm md:text-base"
              >
                {checkingFood ? (
                ...
                {checkingFood ? 'Checking...' : 'Check'}
              </button>
            </div>
            
            {/* Image Preview */}
            {quickCheckImagePreview && (
              <div className="relative">
                <img 
                  src={quickCheckImagePreview} 
                  alt="Food preview" 
          ...
          {/* Quick Check Result - Responsive */}
          {quickCheckResult && (
            <div className="mt-4 bg-gray-50 rounded-xl p-4 md:p-6 space-y-4">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="font-semibold text-lg md:text-xl text-slate-800">
                    {quickCheckResult.foodItem.name}
                  </h3>
                  <p className="text-sm md:text-base text-slate-600">
                    {quickCheckResult.foodItem.quantity}
                  </p>
                </div>
                <div className={`px-3 py-1 rounded-full text-xs md:text-sm font-medium ${
                  quickCheckResult.isHealthy ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                }`}>
                  {quickCheckResult.isHealthy ? '✓ Healthy' : '⚠️ Unhealthy'}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="bg-white p-3 md:p-4 rounded-lg text-center">
                  <Flame className="w-5 h-5 md:w-6 md:h-6 mx-auto mb-2 text-orange-500" />
                  <p className="text-xs md:text-sm text-slate-600">Calories</p>
                  <p className="text-base md:text-lg font-bold text-slate-800">
                    {quickCheckResult.foodItem.nutrition.calories}
                  </p>
                </div>
                <div className="bg-white p-3 md:p-4 rounded-lg text-center">
                  <Activity className="w-5 h-5 md:w-6 md:h-6 mx-auto mb-2 text-blue-500" />
                  <p className="text-xs md:text-sm text-slate-600">Protein</p>
                  <p className="text-base md:text-lg font-bold text-slate-800">
                    {quickCheckResult.foodItem.nutrition.protein}g
                  </p>
                </div>
                <div className="bg-white p-3 md:p-4 rounded-lg text-center">
                  <Apple className="w-5 h-5 md:w-6 md:h-6 mx-auto mb-2 text-green-500" />
                  <p className="text-xs md:text-sm text-slate-600">Carbs</p>
                  <p className="text-base md:text-lg font-bold text-slate-800">
                    {quickCheckResult.foodItem.nutrition.carbs}g
                  </p>
                </div>
                <div className="bg-white p-3 md:p-4 rounded-lg text-center">
                  <Droplets className="w-5 h-5 md:w-6 md:h-6 mx-auto mb-2 text-purple-500" />
                  <p className="text-xs md:text-sm text-slate-600">Fats</p>
                  <p className="text-base md:text-lg font-bold text-slate-800">
                    {quickCheckResult.foodItem.nutrition.fats}g
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white p-3 md:p-4 rounded-lg">
                <p className="text-sm md:text-base text-slate-700">{quickCheckResult.analysis}</p>
              </div>

              {/* Warnings */}
              {quickCheckResult.warnings && quickCheckResult.warnings.length > 0 && (
                <div className="bg-red-50 p-3 md:p-4 rounded-lg border border-red-200">
                  <div className="flex items-start gap-2">
                    <AlertCircle className="w-5 h-5 text-red-600 mt-0.5 flex-shrink-0" />
                    <div className="flex-1">
                      <p className="text-sm md:text-base font-medium text-red-900 mb-2">Warnings:</p>
                      <ul className="text-sm md:text-base text-red-700 space-y-1">
                        {quickCheckResult.warnings.map((warning, idx) => (
              ...
              {quickCheckResult.alternatives && quickCheckResult.alternatives.length > 0 && (
                <div className="bg-green-50 p-3 md:p-4 rounded-lg border border-green-200">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h4 className="text-sm md:text-base font-semibold text-green-900">Healthier Alternatives</h4>
                  </div>
                  <div className="space-y-3">
                    {quickCheckResult.alternatives.map((alt, idx) => (
                      <div key={idx} className="bg-white p-3 md:p-4 rounded-lg">
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            <p className="text-sm md:text-base font-medium text-green-900">{alt.name}</p>
                            <p className="text-xs md:text-sm text-green-700">{alt.description}</p>
                          </div>
                          <p className="text-xs md:text-sm font-medium text-green-900">{alt.nutrition.calories} cal</p>
                        </div>
                        <div className="flex gap-3 text-xs md:text-sm text-green-700 mb-2">
                          <span>P: {alt.nutrition.protein}g</span>
                          <span>C: {alt.nutrition.carbs}g</span>
                          <span>F: {alt.nutrition.fats}g</span>
                        </div>
                        <p className="text-xs md:text-sm text-green-600">✓ {alt.benefits}</p>
                      </div>
                    ))}
                  </div>
                </div>
